...
There are two main constructs dedicated to this: the `Path` class and the `Surveyor` subclasses.

## Paths

A `Path` represents, well, a path that an analyzed program has taken to arrive at a specific state.
...
e = p.survey('Explorer', find=(0x4006ed,), avoid=(0x4006aa,0x4006fd))
e.run()
...
```python
print "The first found path is", p._f
print "The first active path is", p._a
```
