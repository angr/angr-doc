...
def test_api_coverage():
    missing = []
    exclude = ["angr.tablespecs", "angr.service", "angr.analyses.girlscout", "pyvex.vex_ffi",
    ...
    for module in ["angr", "claripy", "cle", "pyvex", "archinfo", "ailment"]:
...
def test_lint_docstrings():
    ...
    if stderr:
        ...
        stderr = b"\n".join(l for l in stderr_lines if
                            b"sphinx_rtd_theme/search.html:21" not in l and b"{{ super() }}" not in l and b"{% endblock %}" not in l)

    if stderr:
        raise Exception(
            "The following warnings were generated while building the API documentation:\n\n%s" % stderr.decode())
